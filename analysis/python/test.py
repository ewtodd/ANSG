...
def format_energy_coarse_gain(value):
    if value.startswith('CHARGESENS_') and 'FC_LSB_VPP' in value:
        parts = value.split('_')
        gain_value = parts[1]
        return f"{gain_value} fC/(LSB x Vpp)"
    return value

# Build the table as it is displayed in CoMPASS, print it, and return it as a DataFrame
def build_table(xml_file):
    ...
    parameter_keys = {
        ...
        'SRV_PARAM_TRG_HOLDOFF': 'Trigger Holdoff (ns)'
    }

    # Parse the XML file to determine the number of channels
    tree = ET.parse(xml_file)
    root = tree.getroot()
    channels = root.find('board').findall('channel')
    channel_count = len(channels)

    # Initialize the channel names
    channel_names = [f'CH{i}' for i in range(channel_count)]
    data = {row_name: {channel: None for channel in channel_names} for row_name in parameter_keys.values()}
